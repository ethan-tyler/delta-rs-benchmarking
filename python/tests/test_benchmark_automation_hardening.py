...
def test_compare_branch_sanitizes_branch_labels_for_cli() -> None:
    ...
    assert re.search(
        r"base_label=\"base-\$\(sanitize_label \"\$\{base_branch\}\"\)\"", script
    )
    assert re.search(
        r"cand_label=\"cand-\$\(sanitize_label \"\$\{candidate_branch\}\"\)\"", script
    )
...
def test_benchmark_workflow_uses_env_vars_for_compare_refs() -> None:
    ...
    assert re.search(
        r"\s+HEAD_REF:\s+\$\{\{ steps\.pr\.outputs\.head_ref \}\}", workflow
    )
