...
def test_pack_plan_rejects_duplicate_suite_profile_entries(
    tmp_path: Path,
) -> None:
    registry_path = tmp_path / "registry.yaml"
    ...
    assert result.returncode == 1
    assert "duplicate suite/profile 'scan[pr-macro]'" in result.stderr
...
def test_pack_audit_fails_when_required_suite_has_extra_non_default_profile(
    tmp_path: Path,
) -> None:
    registry_path = tmp_path / "registry.yaml"
    registry_path.write_text(
...
def test_pack_audit_fails_when_required_suite_is_not_pr_bot_automation(
    tmp_path: Path,
) -> None:
    registry_path = tmp_path / "registry.yaml"
    registry_path.write_text(
        "\n".join(
            [
                ...
                "  scan:",
                "    class: authoritative_macro",
                "    automation_tier: candidate_pr_bot",
                "    default_profile: pr-macro",
                "    readiness: ready",
                "",
                "packs:",
    ...
    payload = json.loads(result.stdout)
    assert payload["ready"] is False
    assert payload["automation_tier_mismatches"] == [
        {
            "suite": "scan",
            "expected_automation_tier": "pr_bot",
            "actual_automation_tier": "candidate_pr_bot",
        }
    ]
